...
def eval_autoregressive(
    ...
    states, actions, physics, reward, remaining = utils.to_torch(batch, device)
    init_obs = states[:, -1]
    states = states[:, :-1]
...
def eval_parallel(
    ...
    batch = next(context_iter)
    states, exp_actions, physics, reward, remaining = utils.to_torch(batch, device)
    init_obs = states[:, -1]
    states = states[:, :-1]
...
def main(cfg):
    ...
    context_loader = make_replay_loader(
        env,
        context_dir,
