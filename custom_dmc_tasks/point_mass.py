...
def reach_multitask(time_limit=_DEFAULT_TIME_LIMIT,
              random=None,
              environment_kwargs=None):
    """Returns the Run task."""
    physics = Physics.from_xml_string(*get_model_and_assets('reach_multitask'))
    task = MultiTaskPointMass(target_id=None, random=random)
...
def reach_top_left(time_limit=_DEFAULT_TIME_LIMIT,
    ...
    task = MultiTaskPointMass(target_id=0, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(physics,
                               task,
                               time_limit=time_limit,
...
def reach_top_right(time_limit=_DEFAULT_TIME_LIMIT,
    ...
    task = MultiTaskPointMass(target_id=1, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(physics,
                               task,
                               time_limit=time_limit,
...
def reach_bottom_left(time_limit=_DEFAULT_TIME_LIMIT,
    ...
    task = MultiTaskPointMass(target_id=2, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(physics,
                               task,
                               time_limit=time_limit,
...
def reach_bottom_right(time_limit=_DEFAULT_TIME_LIMIT,
    ...
    task = MultiTaskPointMass(target_id=3, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(physics,
                               task,
                               time_limit=time_limit,
...
class MultiTaskPointMass(base.Task):
    ...
    """
        if target_id is None:
            ...
            self._targets = [TASKS[target_id][1]]
    ...
    """
        ...
        if len(self._targets) == 1:
            physics.named.data.geom_xpos['target'][:] = self._targets[0]
    ...
    def get_reward(self, physics):
        ...
        for target in self._targets:
            near_target = rewards.tolerance(physics.mass_to_target_dist(target),
                                    bounds=(0, target_size), margin=target_size)
            rews.append(near_target * small_control)
